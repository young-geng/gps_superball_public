...
class PolicyOptCaffe(PolicyOpt):
    ...
    def __getstate__(self):
        ...
        # Save the scaling and bias constant for transfering weight
        if hasattr(self.policy, 'scale') and hasattr(self.policy, 'bias'):
            data = {
                'scale': self.policy.scale,
                'bias': self.policy.bias,
            }
            exp_dir, _ = os.path.split(
                self._hyperparams['weights_file_prefix']
            )
            filename = os.path.join(exp_dir, 'policy_constants_{}.pkl'.format(self.caffe_iter))
