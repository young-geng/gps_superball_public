...
class AlgorithmTrajOpt(Algorithm):
    ...
    def iteration(self, sample_lists):
        ...
        if 'variance_multiplier' in self._hyperparams:
    ...
    def _scale_variance(self):
        for m in xrange(self.M):
            for t in xrange(self.T):
                if np.mean(np.diagonal(self.cur[m].traj_distr.pol_covar[t, :, :])) > self._hyperparams['target_variance']:
                    self.cur[m].traj_distr.pol_covar[t, :, :] *= self._hyperparams['variance_multiplier']
                    self.cur[m].traj_distr.chol_pol_covar[t, :, :] = np.linalg.cholesky(self.cur[m].traj_distr.pol_covar[t, :, :])
                    self.cur[m].traj_distr.inv_pol_covar[t, :, :] = np.linalg.inv(self.cur[m].traj_distr.pol_covar[t, :, :])
