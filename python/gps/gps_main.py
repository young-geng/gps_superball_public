        ...
        if type(self.agent) == AgentSUPERball:
            extra_args = {
                'superball_parameters': {
                    ...
                    'bottom_face': (None if 'bottom_faces'
                        not in self._hyperparams['agent']
                        else self._hyperparams['agent']['bottom_faces'][cond]
                    ),
                    'start_motor_positions': (None if 'start_motor_positions'
                        not in self._hyperparams['agent']
                        else self._hyperparams['agent']['start_motor_positions'][cond]
                    ),
                    'motor_position_control_gain': (None if 'motor_position_control_gain'
                        not in self._hyperparams['agent']
                        else self._hyperparams['agent']['motor_position_control_gain'][cond]
                    ),
                }
            }
    ...
    def _take_policy_samples(self, N=None, test_policy=False):
        ...
        for cond in range(len(self._test_idx)):
            ...
            if type(self.agent) == AgentSUPERball:
                extra_args = {
                    'superball_parameters': {
                        ...
                        'horizon': (None if (not test_policy)
                            or 'policy_test_horizon' not in self._hyperparams['agent']
                            else self._hyperparams['agent']['policy_test_horizon']
                        ),
                        'start_motor_positions': (None if 'start_motor_positions'
                            not in self._hyperparams['agent']
                            else self._hyperparams['agent']['start_motor_positions'][cond]
            ...
            pol_samples[cond][0] = self.agent.sample(
                ...
                verbose=verbose, save=False, noisy=False, **extra_args)
...
def main():
    ...
    if args.targetsetup:
        ...
        if resume_training_itr:
            current_itr = resume_training_itr
