...
class GMM(object):
    ...
    def estep(self, data):
        ...
        for i in range(K):
            ...
            #                           check_finite=False)
            U = _regularized_cholesky(self.sigma[i, :Di, :Di])
    ...
    def update(self, data, K, max_iterations=100):
        ...
        if (not self.warmstart or self.sigma is None or
            ...
            for i in range(K):
                ...
                self.sigma[i, :, :] = _make_positive_definite(sigma)
        ...
        for itr in range(max_iterations):
            ...
            for i in range(K):
                ...
                if self.eigreg:  # Use eigenvalue regularization.
                    ...
                    #         1e-6 * np.eye(Do)
                    self.sigma[i, :, :] = _make_positive_definite(self.sigma[i, :, :])


def _make_positive_definite(M):
    assert len(M.shape) == 2 and M.shape[0] == M.shape[1]    # Make sure M is a square matrix
    if not np.all(M.T == M):
        print 'Correcting symmetry'
        M = 0.5 * (M + M.T)

    assert np.all(M.T == M)

    regularizer = 1e-6
    while regularizer < 1e-1:
        try:
            scipy.linalg.cholesky(M + regularizer * np.eye(M.shape[0]),
                                  check_finite=False)
            break
        except np.linalg.linalg.LinAlgError:
            regularizer *= 3

    return M + regularizer * np.eye(M.shape[0])

def _regularized_cholesky(M):
    return scipy.linalg.cholesky(
        _make_positive_definite(M),
        check_finite=False
    )
